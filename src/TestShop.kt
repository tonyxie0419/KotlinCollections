...
//将List<Customer>转换成hashMap
val customerAfter = shop.customers.fold(hashMapOf<String, Customer>()) {

    //将shop.customers中的name属性作为key，shop.customers中的元素作为value添加到新的hashMap中
    //map[it.name] = it 等同于map.put(it.name, it)
    map, it -> map.put(it.name, it)
//    map[it.name] = it
    map
}
...
val sortedCustomers = listOf(cooper, nathan, bajram, asuka, lucas, riku, reka).map { customerAfter[it] }
...
).mapValues { it.value.map { name -> customerAfter[name] } }